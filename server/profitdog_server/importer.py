"""One-time import of the CSV era.
    ...
    python -m profitdog_server.importer --from . --db profitdog.sqlite3
...
log = logging.getLogger("profitdog_server.importer")
...
def import_file(db: Database, path: Path) -> FileReport:
    ...
    with db.write() as conn:
        existing = conn.execute(
            "SELECT id FROM matches WHERE match_key = %s", (key,)
        ).fetchone()
        if existing is None:
            cursor = conn.execute(
                ...
                " VALUES (%s, NULL, %s, %s, %s, %s, NULL, 'import:csv', %s, %s, %s, 1)"
                " RETURNING id",
                (
                    key,
        ...
        before = conn.execute(
            "SELECT COUNT(*) FROM cash_samples WHERE match_id = %s", (match_id,)
        ).fetchone()[0]
        with conn.cursor() as cursor:
            cursor.executemany(
                ...
                # A re-import must be a no-op, and these rows carry no agent
                # sequence for the envelope gate to recognise, so the natural
                # key is the only thing that can make it one.
...
def import_xp_log(db: Database, path: Path, report: ImportReport) -> None:
    ...
    with path.open(newline="", encoding="utf-8-sig") as handle:
        for raw in csv.DictReader(handle):
            ...
            if match_key:
                found = db.query_one(
                    "SELECT id FROM matches WHERE match_key = %s", (match_key,)
                )
    ...
    if rows:
        with db.write() as conn, conn.cursor() as cursor:
            cursor.executemany(
                "INSERT INTO xp_events (match_id, role, total, delta, source_ts,"
                " observed_at, received_at, ingested_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                rows,
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="profitdog_server.importer")
