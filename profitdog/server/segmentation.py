...
class Segmenter:
        ...
        if game_state == "playing":
            ...
            if not is_real_faction(state.faction):
                ...
                if is_real_faction(candidate):
                    ...
                    conn.execute(
                        "UPDATE matches SET faction = ? WHERE id = ?",
    ...
    def _map(self, conn, fact, state, *, agent_ref, boot_ref, received_at,
        ...
        if name and state.map != name:
            ...
            if state.match_id is not None:
                conn.execute(
                    "UPDATE matches SET map = ? WHERE id = ?", (name, state.match_id)
                )
                pending.api.append(
                    ("match.meta", state.match_key,
    ...
    def _open_match(self, conn, fact, state, *, at, agent_ref, received_at,
        ...
        suffix = 0
        base = key
        while self.db.query_one("SELECT 1 FROM matches WHERE match_key = ?", (key,)):
            suffix += 1
            key = f"{base}-{suffix}"

        cursor = conn.execute(
            "INSERT INTO matches (match_key, agent_ref, started_at, map, faction,"
            " build, source, observed_at, received_at, ingested_at, closed)"
            " VALUES (?, ?, ?, ?, ?, ?, 'agent', ?, ?, ?, 0)",
            (
                key, agent_ref, at.isoformat(), state.map, None, state.build,
                fact.observed_at, received_at, ingested_at,
            ),
        )
        state.match_id = int(cursor.lastrowid)
        state.match_key = key
    ...
    def _close_match(self, conn, state, *, ended_at, reason, pending) -> None:
        ...
        key = state.match_key
        conn.execute(
            "UPDATE matches SET closed = 1, ended_at = ? WHERE id = ?",
