...
def load_readings(db, match_id: int) -> list[Reading]:
    rows = db.query(
        ...
        " WHERE match_id = ? ORDER BY elapsed_sec",
        (match_id,),
    )
    return [
        Reading(
...
def load_overrides(db, match_id: int) -> dict[int, float]:
    rows = db.query(
        ...
        " WHERE scope = 'life_kit_cost' AND match_id = ? AND value IS NOT NULL",
        (match_id,),
    )
...
def derive_all(db, rows: Iterable[Any]) -> list[MatchView]:
    ...
    for r in db.query(
        ...
        f" WHERE match_id IN ({placeholders}) ORDER BY match_id, elapsed_sec",
        ids,
    ):
    ...
    for r in db.query(
        ...
        f" WHERE scope = 'life_kit_cost' AND value IS NOT NULL"
